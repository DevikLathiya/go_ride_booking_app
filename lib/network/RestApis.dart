...
Future<GoogleMapSearchModel> searchAddressRequest({String? search}) async {
  return GoogleMapSearchModel.fromJson(
      await handleResponse(await buildHttpResponse('https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$googleMapAPIKey&components=country:${sharedPref.getString(COUNTRY).validate(value: defaultCountry)}', method: HttpMethod.GET)));
}
...
logOutSuccess() async {
  ...
  launchScreen(getContext, GoRideLoginScreen(), isNewTask: true);
}
