...
Future<void> loginFromFirebase(User currentUser, String loginType, String? accessToken) async {
  ...
  await logInApi(req, isSocialLogin: true).then((value) async {
    ...
    if (value.data!.contactNumber.isEmptyOrNull) {
      //TODO launchScreen(getContext, EditProfileScreen(isGoogle: true), isNewTask: true, pageRouteAnimation: PageRouteAnimation.Slide);
    } else {
      if (value.data!.uid.isEmptyOrNull) {
        await updateProfile(
          ...
          launchScreen(getContext, GoRideHomeScreen(), isNewTask: true, pageRouteAnimation: PageRouteAnimation.Slide);
          //TODO launchScreen(getContext, RiderDashBoardScreen(), isNewTask: true, pageRouteAnimation: PageRouteAnimation.Slide);
        }).catchError((error) {
          log(error.toString());
        });
